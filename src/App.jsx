...
export default function App(){
  ...
  const byMonth=(evs,sc=true)=>MONTHS.map(m=>{const me=evs.filter(e=>e.month===m.id);if(!me.length)return null;return <div key={m.id} style={{marginBottom:"1.8rem"}}><div style={{display:"flex",alignItems:"center",gap:"0.6rem",marginBottom:"0.7rem"}}><span style={{background:J.blue,color:J.white,fontWeight:800,padding:"0.2rem 0.8rem",borderRadius:6,fontSize:"0.8rem"}}>{m.name.toUpperCase()}</span><div style={{height:2,flex:1,background:J.border,borderRadius:2}}/><span style={{fontSize:"0.75rem",color:J.muted,fontWeight:600}}>{me.length} eventos</span></div>{grid(me,sc)}</div>;});
    ...
    <div style={{fontFamily:"'Rethink Sans',sans-serif",background:J.bg,minHeight:"100vh",color:J.text}}>
      <div style={{background:J.blue,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{maxWidth:1280,margin:"0 auto",padding:"1.5rem 2rem 1.2rem",position:"relative",zIndex:1}}>
          ...
          <div style={{marginTop:"1rem"}}><input type="text" placeholder="Buscar evento, artista, cliente, serie..." value={search} onChange={ev=>setSearch(ev.target.value)} style={{width:"100%",maxWidth:460,padding:"0.55rem 1rem",background:"rgba(255,255,255,0.15)",border:"1.5px solid rgba(255,255,255,0.3)",borderRadius:8,color:J.white,fontSize:"0.85rem",outline:"none",boxSizing:"border-box"}}/></div>
